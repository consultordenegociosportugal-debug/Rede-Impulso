...
import { OferecerServicoForm } from "./form";

export default async function OferecerServicoPage() {
  ...
  if (!user) {
    redirect("/entrar?depois=/oferecer-servico");
  }
  ...
  if (tiposNecessarios.length > 0) {
    ...
    if (!count) {
      redirect("/verificacao?depois=/oferecer-servico");
    }
  }

  return <OferecerServicoForm />;
}
